...
void main(void)
{
  ...
  float red   = cos(length(offset) + theta * -2.0f + gTime + 0) * 0.5f + 0.5f;
  float green = cos(length(offset) + theta * -2.0f + gTime + 2) * 0.5f + 0.5f;
  float blue  = cos(length(offset) + theta * -2.0f + gTime + 4) * 0.5f + 0.5f;
