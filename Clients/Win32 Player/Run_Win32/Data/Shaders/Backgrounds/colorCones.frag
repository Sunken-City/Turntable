...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float red   = cos(length(offset) + theta * -2.0f + iTime + 0) * 0.5f + 0.5f;
  float green = cos(length(offset) + theta * -2.0f + iTime + 2) * 0.5f + 0.5f;
  float blue  = cos(length(offset) + theta * -2.0f + iTime + 4) * 0.5f + 0.5f;
